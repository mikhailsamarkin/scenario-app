...
| `import-games.mjs` | Импорт/публикация игры: валидация → slug-уникальность (ED-8) → upload в Supabase Storage (ED-7) → запись `games/{id}` → сборка `game_public` (A-10) → обновление `home_feed`/`sitemap_public`. CLI: `node import-games.mjs --project dev\|prod <game.json> [--dry-run]` |
...
# Unit-тесты валидатора (TC-01..TC-03, A-30, A-4a)
cd tools && node --test test/validate-game.test.mjs
