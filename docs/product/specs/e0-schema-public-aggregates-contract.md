...
Служебный индекс опубликованных сценариев и игр (slug + id) для SSG — чтобы `generateStaticParams` и sitemap строились без N+1 по коллекциям (**A-10b, A-10d**).

| Поле              | Тип                       | Обяз. | Комментарий                                                                 |
| ----------------- | ------------------------ | ----- | --------------------------------------------------------------------------- |
| `scenarioEntries` | array\<{slug, id}> | да    | Все опубликованные сценарии: `slug` — ЧПУ для URL (A-23), `id` — ключ `scenario_public/{id}` (A-24, A-13 |
| `gameEntries`     | array\<{slug, id}> | да    | Все опубликованные игры: `slug` — ЧПУ для URL (A-23), `id` — ключ `game_public/{id}` (A-24, A-13 |
| `contentVersion`  | int                      | да    | <br />                                                                       |
| `updatedAt`       | timestamp                | да    | <br />                                                                       |

> **Изменение контракта (версия 2, A-44):** `scenarioSlugs`/`gameSlugs` (массив string) заменены на `scenarioEntries`/`gameEntries` (массив `{slug, id}`), чтобы SSG мог по ЧПУ `slug` найти `id` для чтения документа (`*_public/{id}`).
...
| 2026-09-07 | AID   | Версия контракта 2 (A-44]: `sitemap_public` — `scenarioSlugs`/`gameSlugs` заменены на `scenarioEntries`/`gameEntries` (массив `{slug, id}`), чтобы SSG по ЧПУ slug находил id для чтения `*_public/{id}` (A-23/A-24/A-13]. |
